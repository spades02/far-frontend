...
const Listing = () => {
  return (
    <>
    <div>
        <img src={photo} alt="listing" className="relative w-full h-80 object-cover"/>
        <Card className={"absolute flex sm:w-6/8 w-7/8 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-7/8 lg:top-1/4 lg:translate-y-1/6 shadow-lg"}>
            <img src={dp} alt='profile photo' className='rounded-l-lg max-h-36 h-full max-w-36 sm:max-w-40 w-full'/>
    ...
    <div className='lg:flex mt-40 md:mt-36 m-8'>
      ...
      <p>
        ...
        scelerisque ligula. Donec non magna nec elit tincidunt
        vestibulum.
      </p>
      <p>
      ...
      <div>
        ...
        <LocationMap latitude={41.8781} longitude={-87.6298} locationName="Chicago, IL" />
      </div>
      
      <Card className={"flex pt-4 md:w-1/2"}>
        <CardHeader>
        <img src={dp} alt="profile picture" className='rounded-full max-h-18 max-w-18'/>
        </CardHeader>
        <CardContent className={"flex flex-col gap-2"}>
          <CardTitle>About Dr. John Smith</CardTitle>
          <CardDescription>
            Dr. John Smith is a board-certified psychiatrist with over 20 years of experience in the field. He specializes in treating anxiety, depression, and other mental health conditions.
            </CardDescription>
            <CardFooter className={"justify-end"}>
              <Link className='text-blue-600 underline' to={"./"}>View Full Profile</Link>  
            </CardFooter> 
        </CardContent>  
      </Card>
      <h1 className='font-bold'>You must complete these in order to reserve:</h1>
      ...
      <Card className={"w-1/3 h-96 grow"} >
          ...
          <CardContent className={"flex flex-col w-full gap-2"}>
            ...
            <ToggleGroup className={"flex-col gap-2"} type="single" variant={"outline"}>
              <ToggleGroupItem value="a" aria-label= "Toggle rotate">
                  <div className='p-2 rounded-lg w-80'>30/09/2022 - 10/10/2022</div>
              </ToggleGroupItem>
    ...
    <div className='flex justify-between m-8'>
      ...
      <Link className='place-self-end' to={"./"}>Report Listing</Link>
      </div>
    </> 
    )
}
